...
zh = 0.5 #Longitudinal momentum fraction of quark(k1+/P+)
...
lims = [[0,30],[0, 100], [0.01, 1]] #integration limits for kper and zf

##################______________________________###########################
#define xsection integrand b independent part 
#__________________________________________________________________#
@vegas.rbatchintegrand
def SIDIS_integrand_2( kper, zf):
    ...
    D_light_sum = 2 * D_light(Pper, zf) #multiply by 2 for antiquark contribution for each flavor of that of quark
    result = H *( Z * M1 + mq**2 * M2 )* dip_proton(Pquark, kper, z1) *D_light_sum *  WS * (1/zf**3) 
    result = np.where(valid, result, 0.0)
    return result
...
def save_results(Pper, mean_T_1, sdev_T_1, mean_T_2, sdev_T_2, mean_T, sdev_T, total_time):
    ...
    filename = os.path.join(output_folder, f"gammaA_T_Q2{Q2}_xbj{xbj}_zh{zh}__sam{sample_id}.csv")
...
sdev_T_list_2= []
##################______________________________###########################
#Main SIDIS calculation loop fir vegas integration
for Q2 in Q2_vec:
    for xbj in xbj_vec:

        print(f"\n{'='*50}")
        print(f"Running A_T Q2={Q2}, xbj={xbj}, zh={zh}, sample={sample_id}")
        print(f"{'='*50}")
        ...
        ##################______________________________###########################
            ...
            #loop intil realtive error is less than desired precision
