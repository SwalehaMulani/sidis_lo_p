...
import dipoleMom_b_dep # Import the dipole momentum b dependent module with cubic interpolation method
...
print("Running SIDIS_T gamma-nucleus with Q2=",Q2, "xbj=", xbj, "zh=", zh)

#sigma0 = 2 * 36.8340746 #mb to GeV^-2 for carlisle fit data MVe model
...
print("TA_b0", TA_b0)
data = pd.read_csv("/users/swanismu/projects/sidislo/gamma_nucleus/x_g_scale/Qs2_red_curve_10pts.csv") # Qs2 values taken from fig 4 of 2311.10491
...
#bk_file = os.path.join("/users/swanismu/projects/sidislo/gamma_nucleus/BK_momentum_space", "BKfit_Sk_data.csv") # for MVe model data fitted to intial conditions
...
def save_results(Pper, mean_T_1, sdev_T_1, mean_T_2, sdev_T_2, mean_T, sdev_T, total_time):
    ...
    filename = os.path.join(output_folder, f"gammaA_T_Q2{Q2}_xbj{xbj}_zh{zh}_MV_{a}.csv")
    ...
    with open(filename, "a", newline="") as f:
        ...
        if not file_exists:
            writer.writerow(["Pper", "mean_T_1", "sdev_T_1","mean_T_2", "sdev_T_2","mean_T", "sdev_T", "time_taken_sec"])

        writer.writerow([Pper,  mean_T_1, sdev_T_1, mean_T_2, sdev_T_2, mean_T, sdev_T, total_time ])
...
#define xsection integrand for transverse photon
#define xsection integrand smaller b, b dependent
for a in a_array:
    for Pper in P_vec:
        ...
        def SIDIS_integrand_1(b,kper, zf):
            ...
            if (a*Pper <= 1.0):
                D_light_sum = 2 * D_light(1, zf)  # use D_light at Q=1 GeV for low Pper values to avoid unphysical large values of fragmentation function at low Q, multiply by 2 for antiquark contribution for each flavor of that of quark
            else:
                D_light_sum = 2 * D_light(a*Pper, zf)
            result =  H *( Z * M1 + mq**2 * M2 )* dipole *D_light_sum * b *  (1/zf**3)
        ...
        lims = [[0,30],[0, 100], [0.01, 1]] #integration limits for b, kper and zf 
        ...
        #initialize variables for mean, sdev 
        mean_T_1, sdev_T_1 = 1e-15, 1e-15

        result_T_1 = integ(integrand_1, nitn=50, neval = 1e6, alpha = 0.5, nproc = 36)
        mean_T_1, sdev_T_1 = result_T_1.mean, result_T_1.sdev
            
        # store results
        mean_T_list_1.append(mean_T_1)
        sdev_T_list_1.append(sdev_T_1)

        final_time_1 = time.time()    
        

        print("Pper = ", Pper, "mean_T_1 = ", mean_T_1, "+/-", sdev_T_1, "Time taken:", final_time_1 - intial_time_1, "seconds")
        #print("--------------------------------------------------")
        
    ##################______________________________###########################
    #define xsection integrand large b and b independent
    #__________________________________________________________________#
        @vegas.rbatchintegrand
        def SIDIS_integrand_2( kper, zf):
            z1 = zh/zf #z1 is longitudinal momentum fraction of quark
            valid = (z1 > 0) & (z1 < 1)
            z2 = 1 - z1 #z2 is fixed by z1 only for this code for simplicity
            arg = Q2 * z1 * z2 + mq**2
            eps = np.sqrt(np.where(valid, arg, 0.0))
            ...
            if (a*Pper <= 1.0):
                ...
                D_light_sum = 2 * D_light(a*Pper, zf)
            result = H *( Z * M1 + mq**2 * M2 )* dip_proton(Pquark, kper, z1) *D_light_sum *  WS *  (1/zf**3) # multiplying with Pper to get d sigma / d Pper format similar to HERA data
            result = np.where(valid, result, 0.0)
            return result
    ...
    ##################______________________________###########################
        ...
        mean_T_2, sdev_T_2 = 1e-15, 1e-15

      
        result_T_2 = integ(integrand_2, nitn=50, neval = 1e6, alpha = 0.5, nproc = 36)
        mean_T_2, sdev_T_2 = result_T_2.mean, result_T_2.sdev
            
        # store results
        mean_T_list_2.append(mean_T_2)
        sdev_T_list_2.append(sdev_T_2)

        final_time_2 = time.time()

        print("Pper = ", Pper, "mean_T_2 = ", mean_T_2, "+/-", sdev_T_2, "Time taken:", final_time_2 - intial_time_2, "seconds")


        ##########################################################################
        #combine both parts

        mean_T = mean_T_1 + mean_T_2
        sdev_T = np.sqrt(sdev_T_1**2 + sdev_T_2**2)
        total_time = (final_time_1 - intial_time_1) + (final_time_2 - intial_time_2)

        
        print("Pper = ", Pper, "mean_T = ", mean_T, "+/-", sdev_T, "Total Time taken:", total_time, "seconds")
        print("--------------------------------------------------")
        save_results(Pper, mean_T_1, sdev_T_1, mean_T_2, sdev_T_2, mean_T, sdev_T, total_time)
