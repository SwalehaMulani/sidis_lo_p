...
def save_results(Pper, mean_T, sdev_T, intial_time, final_time):
    ...
    filename = os.path.join(output_folder, f"gammaP_T_Q2{Q2}_xbj{xbj}_zh{zh}__sam{sample_id}.csv")
...
def SIDIS_integrand(kper, zf):
       ...
       result = H *( Z * M1 + mq**2 * M2 )* dip(Pquark, kper, z1) *D_light_sum * (1/zf**3) # multiplying with Pper to get d sigma / d Pper format similar to HERA data
